...
/**
 * Whether a simulated identity may be used.
 *
 * Refused once World ID is configured: a demo session cannot produce a proof,
 * so every uniqueness rule would fail closed, and "become a verified human by
 * tapping a button" reads as bypassing personhood entirely.
 */
...
export function worldIdPublicConfig(): {
  ...
  return {
    ...
    action: process.env.WORLD_ID_ACTION || "verify",
  };
}
...
export async function verifyWorldIdProof(
  ...
  /**
   * The action the proof must be for. Without it, a proof minted for one poll,
   * thread or claim is replayable on another.
   */
  action?: string,
